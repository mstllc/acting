...
class ActingClass {
  ...
  scene (scenePath) {
    const scene = (this._scenes.get(scenePath)) ? this._scenes.get(scenePath) : new ActingScene(scenePath)
    this._scenes.set(scenePath, scene)
    return scene
  }
  ...
  camera () {
    ...
    this._scenes.forEach((scene, scenePath) => {
      this._camera.add(scenePath)
    })
  ...
  _onFileAdd (filePath) {
    const scene = this._scenes.get(path.dirname(filePath))
    scene.handle('_onFileAdd', filePath)
  }

  _onDirectoryAdd (directoryPath) {
    const scene = this._scenes.get(path.dirname(directoryPath))
